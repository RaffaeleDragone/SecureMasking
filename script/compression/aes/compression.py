...
for file_roi in os.listdir(roi_path):
    ...
    with open(roi_path + "/" + file_roi, "rb") as roi:
        ...
        if encryption_type=='cbc':
            img_encrypted = encrypt_cbc(img_decoded, password)
        if encryption_type=='cfb':
            img_encrypted = encrypt_cfb(img_decoded, password)
        if encryption_type=='ecb':
            img_encrypted = encrypt_ecb(img_decoded, password)
        if encryption_type=='ofb':
            img_encrypted = encrypt_ofb(img_decoded, password)
